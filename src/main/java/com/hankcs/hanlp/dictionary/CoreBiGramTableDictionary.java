...
public class CoreBiGramTableDictionary {
    ...
    static {
        ...
        logger.info("开始加载二元词典" + path[0] + ".table");
        long start = System.currentTimeMillis();
        if (!load(path)) {
            throw new IllegalArgumentException("二元词典加载失败");
        } else {
            logger.info(path[0] + ".table" + "加载成功，耗时" + (System.currentTimeMillis() - start) + "ms");
        }
    }

    static boolean load(String[] paths) {
        String datPath = paths[0] + ".table" + Predefine.BIN_EXT;
...
//        }
    ...
    public static boolean reload() {
        ...
        IOUtil.deleteFile(biGramDictionaryPath[0] + ".table" + Predefine.BIN_EXT);
